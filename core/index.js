...
const WebsiteLogin = require('./src/WebsiteLogin');
...
class SqliteDB extends DB {
  ...
  async get(id) {
    ...
    const data = JSON.parse(secret.get('data'));

    if (data.type === 'WebsiteLogin') {
      return new WebsiteLogin(data);
    }
  ...
  async set(id, secret) {
    await this.Secret.upsert({ id, data: JSON.stringify(secret.toJSON()), });
  }
      ...
      client3.on('ready', async () => {
        ...
        // setTimeout(async () => {
          ...
          client1.on('update', async () => {
            console.log(22, await client1.get(id));
          });

          await client3.set(id, cc);
          // console.log(11, await client3.get(id));
        // }, 5000);
      })
    })
  })
